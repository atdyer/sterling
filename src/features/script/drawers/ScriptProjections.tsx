...
import { POPPER_MODIFIERS } from '../../graph/util';
...
const mapState = (state: RootState) => ({
    atoms: state.scriptSlice.atoms,
    projections: state.scriptSlice.projections,
    unprojected: state.scriptSlice.unprojected
});
...
type ScriptProjectionsProps = ConnectedProps<typeof connector>;

const ScriptProjections: React.FunctionComponent<ScriptProjectionsProps> = props => {

    const projections = props.projections.toArray();

    return (
        <SterlingDrawer.Section title={'Projections'}>
            <div className={'projections'}>
                {
                    projections.map(([sig, atom]) => {
...
export default connector(ScriptProjections);
